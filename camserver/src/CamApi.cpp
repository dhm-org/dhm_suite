 ...
 ******************************************************************************
  ...
  @par Description:  Interface to the camera. Starts the asynchronouse frame acquisition
 ******************************************************************************
 */
#include <stdio.h>
#include <math.h> //ceil
...
AVT::VmbAPI::CameraPtrVector CamApi::GetCameraList()
{
    return m_cameras;
}

int CamApi::FindCameraWithSerialNum(char *sn)
{
    VmbErrorType err;
...
int CamApi::QueryConnectedCameras()
{
    VmbErrorType err;
    AVT::VmbAPI::CameraPtrVector::const_iterator iter;
    std::string cameraID;
...
int CamApi::OpenAndConfigCamera(int cameraidx, int width_in, int height_in, double rate_in, const char *configfile, const char *triggersource)
{
    ...
    if(strcmp(triggersource, "Freerun") == 0) {
        PrepareTrigger(camera, "FrameStart", "Off", triggersource);
    }
    else {
...
int CamApi::StartCameraServer(int frame_port, int command_port, int telem_port)
{
...
int CamApi::StartAsyncContinuousImageAcquisition(int cameraidx, bool logging_enabled, char *rootdir, char *datadir, char *sessiondir)
{
    VmbErrorType err = VmbErrorSuccess;
    AVT::VmbAPI::CameraPtr camera;
    AVT::VmbAPI::FeaturePtr pFeature;
    ...
    m_pFrameObserver = new FrameObserver( m_pCamera, m_circbuff, logging_enabled, (int)width, (int)height, rootdir, datadir, sessiondir, m_verbose);
    // Start streaming
...
void CamApi::SetLogging(bool state)
{
